...
(`_fonds/`) est de la **donnée**, séparée du code — entrées d'encyclopédie,
démonstrations, PGN.
...
2. Puis **confronter cette lecture à l'état git réel** : `git fetch`, et
   ...
   intention ; git décrit un fait. **C'est la confrontation qui révèle ce
   que les fichiers taisent** — une branche orpheline, une PR déjà
   fusionnée, un chiffre périmé.

*(Chemins donnés entre backticks, à lire au besoin — délibérément pas en
`@import`, qui chargerait chaque fichier en entier à chaque session.)*
...
- **Jamais de merge sur `main`** sans le mot explicite de Flavien. Sur
  branche de travail, commit et push sont libres. Ouvrir une PR, oui ;
  la fusionner, non — c'est son clic.
...
- **« consigne »** → graver, au fil de l'eau, toute décision, règle ou
  idée née dans l'échange : règle durable dans `PRINCIPES.md`, chantier
  dans `_fonds/RESTE_A_FAIRE.md` avec ses deux portes, contexte de séance
  dans `_sessions/`. Détail dans `REPRISE.md`.

## Division du travail

Conversation = raisonnement et arbitrage ; Code = fichiers, git, outils
locaux ; **Flavien = la validation à l'écran**, irremplaçable — un rendu
piloté est une proposition, son œil est la validation.
